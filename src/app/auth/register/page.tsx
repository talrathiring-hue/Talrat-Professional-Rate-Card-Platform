...
// ─── Perks list ───────────────────────────────────────────────────────────────
const PERKS = [
  { icon: Zap,      text: 'Live in 5 minutes — no design skills needed' },
  { icon: Bell,     text: 'WhatsApp alerts the moment a client contacts you' },
  { icon: BarChart2,text: 'Analytics — see who viewed and where they came from' },
  { icon: Share2,   text: 'Share kit — LinkedIn, WhatsApp, cold email templates' },
]
  ...
  return (
    <div className="text-center py-4 animate-fade-in">
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 mb-5 text-left space-y-2">
        <p className="text-xs font-medium text-slate-500 mb-2">Three steps to your account:</p>
        {[
          'Open the email from talrat',
          'Click the sign-in link',
          'Start building your rate card',
        ].map((s, i) => (
          <div key={i} className="flex items-center gap-2.5">
            <div className="w-5 h-5 rounded-full bg-brand-600 text-white text-[10px] font-bold flex items-center justify-center flex-shrink-0">
              {i + 1}
            </div>
      ...
      <p className="text-xs text-slate-400 mb-4">
        Expires in 10 minutes · Check spam if not received
      </p>
      <button
        onClick={onReset}
...
export default function RegisterPage() {
  ...
  return (
    <>
      {/* ── Heading ── */}
      <div className="mb-6">
        <div className="inline-flex items-center gap-2 px-3 py-1 bg-brand-50 border border-brand-100 rounded-full text-xs font-medium text-brand-700 mb-4">
        ...
        <p className="text-slate-500 text-sm">
          Join thousands of Indian freelancers on talrat
        </p>
      </div>
        ...
        {emailSent ? (
          ...
          <>
            ...
            <button
              ...
              disabled={isLoading}
              className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-white border border-slate-200 rounded-xl text-slate-700 font-medium text-sm hover:bg-slate-50 hover:border-slate-300 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-sm mb-4"
            >
              {googleLoading
